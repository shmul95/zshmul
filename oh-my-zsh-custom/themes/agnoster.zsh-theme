...
: ${AGNOSTER_GIT_CLEAN_BG:=202}
: ${AGNOSTER_GIT_DIRTY_FG:=white}
: ${AGNOSTER_GIT_DIRTY_BG:=202}
...
prompt_virtualenv() {
  if [[ -n "$VIRTUAL_ENV" && -n "$VIRTUAL_ENV_DISABLE_PROMPT" ]]; then
    # Only show the environment name without parentheses
    prompt_segment "$AGNOSTER_VENV_BG" "$AGNOSTER_VENV_FG" "${VIRTUAL_ENV:t}"
  fi
}
